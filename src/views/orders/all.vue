<template lang="pug">
.orders
  view-header(title="全部订单", subtitle="商城实物商品交易订单查询及管理")
      ...
      .table-list(v-else)
        .table-item(v-for="item,index in orderList")
          .table-item-header
            ...
            .table-item-handle
              el-button(v-if="item.order_status === '待确定'", type="primary", size="small", @click="confirmOrder(item.order_id)") 确定订单
              el-button(v-if="item.order_status === '待发货'", type="primary", size="small", @click="send(item.order_id)") 去发货
              el-button(v-if="item.order_status === '待付款' || item.order_status === '待确定'", type="danger", size="small", @click="closeOrder(item.order_id)") 关闭订单
          el-table(
            style="width: auto",
...
import { orderList, confirmOrder, sendGoods, supplierBrand, deliveryType } from 'api'
export default {
  name: 'allOrders',
  data () {
    return {
      ...
      filter: [
        ...
        [
          { name: '订单完成时间:', type: 'date', key: 'endTime', endTime: [] },
  ...
  methods: {
    ...
    changeCurrent (page) {
      ...
      this.$router.replace({ name: 'allOrders', query: { ...this.$route.query, page: this.currentPage } })
    },
    prev () {
      this.currentPage--
      this.$router.replace({ name: 'allOrders', query: { ...this.$route.query, page: this.currentPage } })
    ...
    async getOptions () {
      try {
        ...
        // 交货期列表
        const { data: { delivery_type } } = await this.$axios.get(deliveryType) // eslint-disable-line
        this.brand = brand
        this.delivery_type = delivery_type // eslint-disable-line
      } catch ({ msg }) {
        this.$message.error(msg)
      }
    },
    ...
    closeOrder (id) {
      this.$modal.show('closeOrder', { orderId: id })
    },
    search () {
      let query = {}
      this.filter.map(row => row.map(v => {
        if (v.key === 'endTime' && !v[v.key]) v[v.key] = []
        if (v.key === 'payTime' && !v[v.key]) v[v.key] = []
        if (v[v.key]) query[v.key] = v[v.key]
      }))
      query['order_end_stm'] = query['endTime'][0]
      query['order_end_etm'] = query['endTime'][1]
      query['order_pay_stm'] = query['payTime'][0]
      query['order_pay_etm'] = query['payTime'][1]
      Object.keys(query).forEach(v => {
        if (v === 'endTime' || v === 'payTime' || !query[v]) delete query[v]
      })
      this.$router.replace({ name: 'allOrders', query: { ...query, order_status: this.$route.query.order_status } })
    },
    async initFilter () {
      try {
        this.filter.forEach(row => row.forEach(v => {
          ...
          if (v.key === 'endTime') {
            v[v.key] = [ this.$route.query['order_end_stm'] || '', this.$route.query['order_end_etm'] || '' ]
          } else if (v.key === 'payTime') {
    ...
    reset () {
      this.$router.replace({ name: 'allOrders', query: { order_status: 'ALL' } })
    },
    send (id) {
      this.$router.push({name: 'orderDetail', params: { id }, query: {editLog: 1}})
    },
