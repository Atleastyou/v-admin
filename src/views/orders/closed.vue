<template lang="pug">
.orders
  view-header(title="已关闭", subtitle="商城实物商品交易订单查询及管理")
...
export default {
  ...
  methods: {
    ...
    changeCurrent (page) {
      ...
      this.$router.replace({name: 'closed', query: {...this.$route.query, page: this.currentPage}})
    },
    prev () {
      this.currentPage--
      this.$router.replace({name: 'closed', query: {...this.$route.query, page: this.currentPage}})
    ...
    search () {
      ...
      this.$router.replace({name: 'closed', query: { ...query, order_status: this.$route.query.order_status }})
    },
    async initFilter () {
      try {
    ...
    reset () {
      this.$router.replace({name: 'closed', query: {order_status: 'CLOSED'}})
    },
