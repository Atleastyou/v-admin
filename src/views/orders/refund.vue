<template lang="pug">
.orders
  view-header(title="退款中", subtitle="商城实物商品交易订单查询及管理")
...
import { orderList, sendGoods, supplierBrand, deliveryType } from 'api'
export default {
  name: 'refund',
  data () {
    return {
  ...
  methods: {
    ...
    changeCurrent (page) {
      ...
      this.$router.replace({name: 'refund', query: {...this.$route.query, page: this.currentPage}})
    },
    prev () {
      this.currentPage--
      this.$router.replace({name: 'refund', query: {...this.$route.query, page: this.currentPage}})
    ...
    search () {
      ...
      this.$router.replace({name: 'refund', query: { ...query, order_status: this.$route.query.order_status }})
    },
    async initFilter () {
      try {
    ...
    reset () {
      this.$router.replace({name: 'refund', query: {order_status: 'WAITRETURN'}})
    },
